...
- A **Tailscale sidecar** for Postgres lives in `docker-compose.tailnet.yml` (overlay, not the base file). The base `docker-compose.yml` is teammate-safe — `docker compose up` brings up a normal local stack with postgres on the docker bridge. The host machine opts in by setting `COMPOSE_FILE=docker-compose.yml:docker-compose.tailnet.yml` in its `.env`, which adds the `tailscale` service, sets `postgres` to `network_mode: "service:tailscale"`, and aliases tailscale as `postgres` on the docker network so `DATABASE_URL` is unchanged. Teammates then query the host's DB at `flat-chat-db.<tailnet>.ts.net:5432` over the tailnet. See `agent-compound-docs/decisions/shared-dev-database.md`.
