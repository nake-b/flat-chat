...
- Postgres is fronted by a **Tailscale sidecar** (`tailscale` service in `docker-compose.yml`). `postgres` uses `network_mode: "service:tailscale"`, so it lives inside the tailscale container's network namespace and is reachable to teammates at `flat-chat-db.<tailnet>.ts.net:5432` over the tailnet — not publicly. The tailscale service is aliased as `postgres` on the docker network, so `backend`/`ingestion` keep using `postgres:5432` and `DATABASE_URL` is unchanged. Port 5432 is bound to `127.0.0.1` on the host for local GUI tools. Stack will not start without `TS_AUTHKEY` (compose fails fast via `${TS_AUTHKEY:?...}`). See `agent-compound-docs/decisions/shared-dev-database.md`.
