...
- **Map overlays** — `SessionState.map_overlays` holds geometries the agent draws (the Spree, a U-Bahn line, a Bezirk, the ring). `search_apartments` auto-draws the place/line it filters on (`near_place_ref` / `transit.lines`); `show_on_map(place_ref | transit_line)` pins one without filtering; `clear_map_overlays` wipes them. Geometry resolves via `PlaceService.overlay_geometry` (named_places) and the display-only `TransitRouteService` (GTFS route shapes — separate from the stop-based transit *filter*). Backend sets semantics only (`kind`/`label`/`geojson`); the frontend owns appearance (`state/overlayStyles.ts`) and dismissal (`OverlayLegend` + `dismissOverlay`). Full record: `agent-compound-docs/decisions/map-overlays.md`.
