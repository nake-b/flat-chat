...
- **Resolved: lens-aware clusters.** A "lens" (an active map visualization channel — `marker_channel`, e.g. commute time) used to repaint the individual pins but NOT the `CLUSTER_LAYER`, which stayed red-by-`point_count` and drowned the heatmap at city zoom. Now `buildClusterPaint(channel)` mirrors `buildPinPaint`: when a heatmap lens is active, clusters colour by the aggregated **mean** value (MapLibre `clusterProperties`: `sum_value` / `n_valued`, ramp over `sum_value / max(n_valued, 1)`, neutral "no-data" colour when `n_valued = 0`) via the SAME `state/channelStyles.ts` ramp (shared `rampColorExpression` helper); with no lens, today's count-based red. Cluster *size* always stays count-based. Rejected: turning clustering off under a lens (loses density, thousands of overlapping pins at zoom-out). Hexbin/H3 is a deferred future view, not a rejection — see the Deferred section.
