...
- Conversation lifecycle: `POST /api/conversations` (create session, id doubles as AG-UI `thread_id`), `GET /api/conversations/{id}/messages` (history reload — read-only), `GET /api/conversations/{id}/state` (latest `SessionState` snapshot — the reload-recovery primitive). All ownership-checked against `get_user_id()` (a foreign conversation 404s, not 403). Conversations are durable in Postgres (`app.*`). See [`session-persistence.md`](agent-compound-docs/decisions/session-persistence.md)
