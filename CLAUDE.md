...
- **Spatial neighbour tables to restore old search precision.** The gold-platinum refactor moved geo-context work to ingest-time but narrowed several filters along the way: `transit.lines` / `transit.stop_name` see only the nearest stop; `school` and `hospital` filters silently ignore `distance` / `school_type` / `tier`. Planned fix: per-family neighbour tables (`listings_nearby_transit`, `listings_nearby_schools`, `listings_nearby_hospitals`) populated by gold ETL with top-K=5 ∪ within-R rows. Full plan, trade-offs, sources, and phased steps in [`spatial-neighbor-tables.md`](agent-compound-docs/decisions/spatial-neighbor-tables.md). Same doc also captures two non-table fixes: `max_noise` NULL semantics (one-line revert to old optimistic-include) and a gold-completeness drift probe.
