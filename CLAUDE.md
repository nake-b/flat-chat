...
- **Resolved: transit-feed freshness (clamp + label).** A lapsed MOTIS timetable window used to make the transit lens return ~0 reachable stops → all-grey pins. The backend now reads the loaded window from MOTIS `/metrics` (`fetch_transit_feed_window` in `routing/service.py`), **clamps** the commute departure into it, and **labels** stale results ("schedule as of <date>" on `TravelTimeFilter` → `apply_travel_time` prose + `LensLegend` + `GET /api/health?extended=true`'s `transit_feed`). Car is date-independent (untouched). Full record: [`travel-time-routing.md`](agent-compound-docs/decisions/travel-time-routing.md) §Freshness.
