...
nginx/               → Reverse proxy. /api/auth + /api/conversations
                       + /api/agent (SSE) + /api/listings + /api/health + /tiles/.
...
- Conversation lifecycle: `POST /api/conversations` (create session, id doubles as AG-UI `thread_id`), `GET /api/conversations` (list — powers the sidebar; per-user, excludes empty threads, ordered by `updated_at` DESC; titles are LLM-generated by a background task fired on the first completed turn), `GET /api/conversations/{id}/messages` (history reload — read-only), `GET /api/conversations/{id}/state` (latest `SessionState` snapshot — the reload-recovery primitive), `DELETE /api/conversations/{id}` (hard delete; cascades to messages + session_state via FK; 204 on success). All ownership-checked against `get_user_id()` (a foreign conversation 404s, not 403). Conversations are durable in Postgres (`app.*`). See [`session-persistence.md`](agent-compound-docs/decisions/session-persistence.md)
...
- **State emission is structural — tools just mutate `deps.state`.** Pydantic AI's AG-UI adapter is one-directional (it sets `deps.state` from the envelope at run start but never pushes mutations back); the only way state reaches the UI is a `BaseEvent` in a tool's `ToolReturn.metadata`. Rather than make every tool remember to emit, the toolset is wrapped in `StateEmittingToolset` (`chat/tools/emission.py`, a `WrapperToolset`) that intercepts `call_tool`, diffs `deps.state` before/after, and auto-attaches a `StateSnapshotEvent` when it changed (idempotent; pure-query tools emit nothing). So tool authors only mutate state — emission can't be forgotten. The reverse direction (UI → agent) goes through `chat/service.py:merge_incoming_state`, the single edit-site for frontend-owned fields (`active_id`, `active_listing_detail`, overlay dismissals); persisted server state wins for everything else. See `agent-compound-docs/decisions/map-overlays.md` and `frontend-stack.md`.
- **Map overlays** — `SessionState.map_overlays` holds geometries the agent draws (the Spree, a U-Bahn line, a Bezirk, the ring). `search_apartments` auto-draws the place/line it filters on (`near_place_ref` / `transit.lines`); `show_on_map(place_refs=[…], transit_lines=[…])` pins them without filtering; `hide_on_map(targets=[…])` removes specific ones by label/id; `clear_map_overlays` wipes all (show/hide take lists so a batch is one atomic state mutation — parallel single-arg calls would race on `deps.state`). Geometry resolves via `PlaceService.overlay_geometry` (named_places) and the display-only `TransitOverlayService` (`search/transit_overlays.py` — GTFS route shapes + served stations from `world.transit_stops`, attached as `MapOverlay.points` — separate from the stop-based transit *filter*). The overlay vocabulary (`MapOverlay`/`OverlayPoint` + `OVERLAY_*` constants) lives in the leaf `listings/overlays.py` so both `search/` resolvers and `chat/` can import it. Backend sets semantics only (`kind`/`label`/`geojson`/`points`); the frontend owns appearance (`state/overlayStyles.ts` — colours, breathing area overlays, transit station dots + line badges) and dismissal (`OverlayLegend` + `dismissOverlay`). Full record: `agent-compound-docs/decisions/map-overlays.md`.
...
- LLM provider selection lives in `chat/providers/__init__.py:build_chat_model()` — the single provider seam. Three providers wired today: OpenAI (standard, non-Azure), Anthropic-direct (native prompt caching), and Azure OpenAI. The orchestrator only checks key presence; each builder (`providers/openai.py`, `providers/anthropic.py`, `providers/azure.py`) owns its own validation and provider-specific model settings (cache breakpoints live in `anthropic.py`, not on the Agent). Preference order when multiple keys are set: **OpenAI > Anthropic > Azure** (the first whose key is set wins). Note: the OpenAI builder runs on SDK defaults for now — it is NOT yet hardened with the custom stall-timeout/retry client `anthropic.py` carries (revisit if its egress corrupts the way api.anthropic.com does). See the docstring in `providers/__init__.py` for the four-layer rule and the "add a provider" recipe
...
- **Named-place search + admin/ring geo fields (geo-context v2).** A two-tool flow handles "near a *specific* named place": `locate_place(name)` → opaque `place_ref` → `search_apartments(near_place_ref=…)` → geometry-precise `ST_DWithin` against the resolved shape (correct for the Spree line and the TU campus polygon, not just a centroid). `place_ref` is composed by the ingestion-owned `world.named_places` VIEW (UNION over curated_places ∪ landmarks ∪ parks ∪ water ∪ schools ∪ kitas ∪ hospitals ∪ transit_stops; the backend reads it via `PlaceService` and knows only the token format). `locate_place` returns a **rich candidate menu** — each labelled with kind, shape (area/line/point via `ST_Dimension`) and Ortsteil locality — and the agent picks the right one (prefer an area over a coincident point; ask which when several distinct campuses match) rather than blindly taking rank #1. `locate()` widens matching with word similarity (`%>`) so abbreviations resolve, and ranks `priority DESC` (the `curated_places` arm is `priority=1`) → score bucket → dimension. `curated_places` is the editorial university-campus gazetteer with footprint-derived-but-frozen geometry (authored via `geo_context.author_campuses` → `university_seed.yaml`); HU is 3 campuses so it stays a "which one?" question. Generic "near *a* park/lake/kita" stays a junction-table category filter (no name field). See `named-place-search.md` (§ July 2026 / issue #38). New gold fields on `listings_geo_context`: `inside_ring` (Umweltzone membership), `listing_bezirk` / `listing_ortsteil` (ALKIS polygon `ST_Covers`), `noise_total_lnight` (the noise table was renamed `street_noise_2022` → `strategic_noise_2022`). District search OR-unions `Listing.district ∪ listing_bezirk ∪ listing_ortsteil`. See `named-place-search.md` + `bezirk-ortsteil-resolution.md`.
...
- **TODO — CopilotKit Web Inspector is hidden.** Because we use `selfManagedAgents` (direct AG-UI via `HttpAgent`) instead of a CopilotRuntime middleware, `/api/agent/info` returns 422 and the inspector renders a "Runtime error" banner. `services/frontend/src/main.tsx` passes `showDevConsole={false}` *and* a `MutationObserver` actively removes any `<cpk-web-inspector>` element from the DOM. To re-enable it for live debugging of AG-UI flows we either (a) add a CopilotRuntime middleware (Next.js-only today; Vite would need a custom server) or (b) stub `GET /api/agent/info` with a payload that satisfies CopilotKit's parser. Either is a non-trivial side project; the inspector is *not* needed for the chat to function — Phoenix at `:6006` covers most of the debugging needs.
- **GDPR / personal-data handling** — the compliance posture is recorded in [`agent-compound-docs/decisions/gdpr.md`](agent-compound-docs/decisions/gdpr.md): scraped listing data stores **no poster PII** (enforced by `strip_pii` + `redact_freetext`, see [`poster-pii-minimization.md`](agent-compound-docs/decisions/poster-pii-minimization.md)); user data (email, transcript, typed location) is stored under **contract necessity** with per-user isolation + hard-delete erasure; LLM processing runs EU-resident (Azure OpenAI EU Data Zone). A user-facing privacy notice ships as a static page at `/privacy.html` (`services/frontend/public/privacy.html`), linked from the login screen + the account menu.
...
- **Resolved (0008): B′ — transit stops in the `named_places` gazetteer.** `locate_place` (and therefore `apply_travel_time_lens`'s anchor) now resolves arbitrary S/U-Bahn/tram/bus stations, not just curated landmarks. A **deduped `transit_stop` arm** was added to the `world.named_places` view (`GROUP BY name` + `ST_Centroid(ST_Collect(geom))` collapses the per-platform rows into one station point), plus a `GIN(name gin_trgm_ops)` index on `transit_stops`. The arm forced `src_id` to TEXT on every view arm (a GTFS `stop_id` is colon-laden, e.g. `de:11000:900100003`), which rippled into `listings/models.py` (`src_id` column → `Text`), `_parse_place_ref` (split on the FIRST `:`, keep the text remainder), and an `overlay_geometry` `kind`-guard that draws the station point itself instead of snapping to a nearby building footprint. Reuses the existing kind-tagged `place_ref` abstraction (a stop is name-shaped — name → point — like the other gazetteer sources; a transit *route* correctly stays a separate resolver because it's line-name → shape). Rationale + rejected alternatives in [`travel-time-routing.md`](agent-compound-docs/decisions/travel-time-routing.md); overlay precedent in [`map-overlays.md`](agent-compound-docs/decisions/map-overlays.md).
- **Resolved: lens-aware clusters.** A "lens" (an active map visualization lens — `marker_lens`, e.g. commute time) used to repaint the individual pins but NOT the `CLUSTER_LAYER`, which stayed red-by-`point_count` and drowned the heatmap at city zoom. Now `buildClusterPaint(lens)` mirrors `buildPinPaint`: when a heatmap lens is active, clusters colour by the aggregated **mean** value (MapLibre `clusterProperties`: `sum_value` / `n_valued`, ramp over `sum_value / max(n_valued, 1)`, neutral "no-data" colour when `n_valued = 0`) via the SAME `state/lensStyles.ts` ramp (shared `rampColorExpression` helper); with no lens, today's count-based red. Cluster *size* always stays count-based. Rejected: turning clustering off under a lens (loses density, thousands of overlapping pins at zoom-out). Hexbin/H3 is a deferred future view, not a rejection — see the Deferred section.
- **Resolved: transit-feed freshness (clamp + label).** A lapsed MOTIS timetable window used to make the transit lens return ~0 reachable stops → all-grey pins. The backend now reads the loaded window from MOTIS `/metrics` (`MotisClient.feed_window` in `routing/motis.py`), **clamps** the commute departure into it, and **labels** stale results ("schedule as of <date>" on `TravelTimeLens` → `apply_travel_time_lens` prose + `LensLegend` + `GET /api/health?extended=true`'s `transit_feed`). Car is date-independent (untouched). Full record: [`travel-time-routing.md`](agent-compound-docs/decisions/travel-time-routing.md) §Freshness.
- **TODO — routing data refresh is manual.** `scripts/prep-routing.sh` re-downloads the Berlin OSM + VBB GTFS, rebuilds the OSRM/MOTIS graphs (`first_day: TODAY`), and restarts the engines — but nothing runs it on a schedule. The VBB feed carries a short service calendar, so the transit window lapses within ~a week and the lens starts clamping/labelling. When we schedule ETL (none is scheduled for the MVP), routing refresh should be its **own** nightly host-cron job — separate cadence + failure domain from the listings ETL, per the [`geo-context-pipeline.md`](agent-compound-docs/decisions/geo-context-pipeline.md) "cron at the deployment layer" convention.
...
- **(c) Hexbin / H3 density-heatmap lens** — the gold-standard value-over-space view, deferred as its own viz project. Under an active lens, aggregate the markers into hexagonal bins (precompute H3 cell → mean lens value, client- or server-side) and render a `fill` layer at a zoom-dependent H3 resolution, instead of the count-clusters we ship today. Gives a true continuous heatmap with no overlapping-pin clutter at zoom-out. Why it's deferred, not done: MapLibre has no native hexbin/H3 support (needs `h3-js` + a turf/geojson bin layer, or a server-side tile of pre-binned aggregates), and it must reconcile with the pin/selection UX (a hexbin hides individual pins, so we'd need a zoom threshold that swaps hexbins ⇄ pins, or a toggle). Pairs with the existing lens-aware clusters (which solve the same "heatmap drowns at zoom-out" problem at the cluster level). See the lens-aware-clusters resolution above.
...
Install (Pydantic AI **v2**): `pydantic-ai-slim[ag-ui,anthropic,openai]>=2.0,<3.0`. v2's core no longer bundles every provider — declare only the extras you use (`ag-ui` for the `AGUIAdapter` on `/api/agent`, `anthropic` + `openai` — the latter serves both our OpenAI and Azure providers). Do NOT add the `pydantic-ai` metapackage; it pulls every provider extra back in. See [`agent-compound-docs/decisions/pydantic-v2-migration.md`](agent-compound-docs/decisions/pydantic-v2-migration.md).
...
ever importing the `agent` object — kills the `agent.py ↔ tools/core.py` cycle. In
...
class CoreCapability(AbstractCapability[ChatDeps]):
    def get_toolset(self) -> AgentToolset[ChatDeps] | None:
        return StateEmittingToolset(toolset)  # auto-emits STATE_SNAPSHOT
