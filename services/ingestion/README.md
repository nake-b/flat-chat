...
- **Kleinanzeigen listings don't expose street addresses** — only `postcode + district` + lat/lng. The silver transformer reflects that.
- **Geocoding fallback fills missing coordinates — part of `transform()`.** Most listings arrive geocoded, but some sources (notably **wohninberlin**) never expose a point — those land with NULL `location` and are invisible to search + gold geo-context (both gate on `location IS NOT NULL`). The tail of `silver.transform()` runs a best-effort pass (`_geocode_missing`) that geocodes the address of every `location IS NULL` listing through an open API (Nominatim by default), validates the result is inside Berlin (`clean_berlin_coords`), and writes `latitude`/`longitude`/`location` back. Only `location IS NULL` rows are touched, so it's idempotent and the silver UPSERT (`upsert.py`) preserves the backfilled point across re-runs. Config: `NOMINATIM_BASE_URL` / `GEOCODER_USER_AGENT` / `GEOCODER_RATE_LIMIT_S` (public Nominatim is ≤1 req/s — point the base URL at a self-hosted/Photon instance for volume). Postcode/district-only addresses geocode to an approximate area centroid.
