...
`MapOverlay` carries only `id` / `kind` / `label` / `geojson` / `origin` / `points`. **Appearance lives entirely on the frontend** in `state/overlayStyles.ts`, keyed by `(kind, geometry type)` — solid colored transit lines (Berlin U/S-Bahn palette by label), translucent ring/bezirk/place-polygon fills, water-blue river lines. This mirrors how marker paint lives in `MapPane.tsx` and status labels in `toolStatus.ts`. No `style_hint` on the wire — it would be a second, drifting source of truth. (GTFS `route_color` exists in `world.transit_routes` but is intentionally *not* forwarded — keeping the backend semantics-only.)

Two motion/appearance details the frontend adds, all driven by one `prefers-reduced-motion`-gated `requestAnimationFrame` loop in `OverlayLayer` (constants in `overlayStyles.ts`): **area overlays breathe** (a slow halo + fill opacity pulse) so a drawn region reads as "active"; and **transit overlays render their served stations** (`MapOverlay.points`) as dots with a pulsing aura, plus a **BVG-style line badge** ("U8", a runtime canvas image in the line's palette colour) at each end. Overlays mount `beforeId="clusters"` so they always sit beneath the apartment pins. None of this touches the wire — the backend still ships semantics only.
...
- **Auto:** `search_apartments` rebuilds search-derived overlays from its spatial anchors (`near_place_ref` → place geometry; `transit.lines` → each line's geometry), mirroring the search tool's own asymmetry. These are `origin="search"` and **replaced** each search — so dropping a filter (re-running search without it) drops its overlay automatically; no parallel tool call or removal hook needed.
- **Explicit/proactive:** `show_on_map(place_refs=[…], transit_lines=[…])` pins overlays (`origin="pinned"`) that **survive** subsequent searches until removed or dismissed. Args are **lists** (not single refs) so a batch draw is one atomic `deps.state` mutation → one snapshot; parallel single-arg calls would race on the shared state and emit inconsistent snapshots.
