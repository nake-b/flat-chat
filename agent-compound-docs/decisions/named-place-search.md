...
`landmarks` is the one named class with no pre-existing source table. It is fed from two sources:

- **ALKIS** (`alkis_gebaeude`, `source='alkis'`) — Berlin's official building cadastre. Richer than expected: Fernsehturm, Siegessäule, Reichstag, and TU Berlin all carry named footprints. ALKIS is the seed.
- **OSM** (Overpass, `source='osm'`) — fills the *free-standing Bauwerk* gap that ALKIS's building-footprint model misses: Brandenburger Tor, the Olympiastadion bowl, bridges. Tags ingested: `historic=monument|memorial`, `man_made=tower|bridge`, `tourism=attraction`, `leisure=stadium`. Native geometry preserved (points, lines, polygons), appended after the ALKIS seed with `source='osm'` + a `category` derived from the tag.
